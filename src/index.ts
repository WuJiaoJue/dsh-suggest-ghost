...
export function apply(ctx: Context, config: GenerateConfig): void {
  ...
  const pushToClient = (
    ...
    void settingsSvc.update(SUGGEST_GHOST_NAMESPACE, { _push: JSON.stringify(payload) }).catch((error: unknown) => {
  ...
  ctx.inject(['sessionProjections'], (projectionCtx) => {
    projectionCtx.sessionProjections.register<typeof PROJECTION_KEY, SuggestGhostProjection>({
      key: PROJECTION_KEY,
