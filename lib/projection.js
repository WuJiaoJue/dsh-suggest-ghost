...
/** 空日志初始状态：无建议、无热度。 */
export function initSuggestGhostProjection() {
 ...
 */
export function applySuggestGhostProjection(state, event) {
    if (event.type === HOT_INDEX_EVENT) {
        const hot = event.data;
        const currentTurn = state.suggestion?.turn ?? -1;
        if (hot.turn < currentTurn)
            return state;
        return { suggestion: state.suggestion, hot: hot.entries };
    }
    if (event.type !== SUGGEST_EVENT)
        return state;
    const suggested = event.data;
    const current = state.suggestion;
    // 忽略旧于当前建议的乱序事件（last-wins）。
    if (current !== null && suggested.turn <= current.turn)
        return state;
    return {
        suggestion: {
            version: 1,
