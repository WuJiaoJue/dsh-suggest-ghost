 ...
 * 跳过空白、系统提醒包装块与超长文本。
 */
function userTextOfEvent(event) {
...
export function apply(ctx, config) {
    ...
    ctx.inject(['settings'], (settingsCtx) => {
        try {
            ...
            const unwatchOps = scope.watch((next, prev) => {
                if (next._ops === prev._ops)
                    return;
                const ops = parseHotnessOps(next._ops);
                if (ops.length === 0 || svc === undefined)
                    return;
                ...
                for (const op of ops) {
                    ...
                    if (ring === undefined)
                        continue; // 未播种（该会话自插件启动以来无事件）：回空，等其下次事件
                    pushToClient({
    ...
    const ringPush = (sessionId, text) => {
        ...
        if (ring[ring.length - 1] === text)
            return; // 相邻重复只计一次
        ring.push(text);
        if (ring.length > HISTORY_PUSH_CAP * 2)
            ring.splice(0, ring.length - HISTORY_PUSH_CAP * 2);
    };
