...
class MinSeqHeap {
    ...
    push(text, seq) {
        ...
        while (i > 0) {
            const parent = (i - 1) >> 1;
            if (this.heap[parent].seq <= this.heap[i].seq)
                break;
        ...
        while (this.heap.length > 0) {
            ...
            if (this.heap.length > 0) {
                ...
                for (;;) {
                    ...
                    const r = l + 1;
                    let smallest = i;
                    if (l < n && this.heap[l].seq < this.heap[smallest].seq)
                        smallest = l;
                    if (r < n && this.heap[r].seq < this.heap[smallest].seq)
                        smallest = r;
                    if (smallest === i)
                        break;
...
export class HotnessTable {
     ...
     */
    restoreEntry(text, count, pinned = false) {
        const clean = text.trim();
        if (clean === '' || !Number.isInteger(count) || count < 1 || clean.length > HOT_TEXT_MAX_CHARS)
            return;
        const prev = this.table.get(clean);
        if (prev === undefined) {
     ...
     */
    addEntry(text, pinned = false) {
        const clean = text.trim();
        if (clean === '' || clean.length > HOT_TEXT_MAX_CHARS)
            return false;
        const prev = this.table.get(clean);
        if (prev === undefined) {
            const entry = { count: 1, lastSeq: Number.NEGATIVE_INFINITY, pinned };
            this.table.set(clean, entry);
            this.heap.push(clean, Number.NEGATIVE_INFINITY);
            if (this.table.size > HOT_TABLE_MAX_ENTRIES)
                this.evictLeastRecent();
    ...
    evictLeastRecent() {
        ...
        if (this.heap.size > this.table.size * 4 + 64)
            this.compactHeap();
    }
    ...
    consume(event, sessionId) {
        ...
        if (text === '' || text.length > HOT_TEXT_MAX_CHARS)
            return;
    ...
    snapshot(limit) {
        ...
        return slice.map(({ text, count, pinned: isPinned }) => isPinned ? { text, count, pinned: true } : { text, count });
    }
    /** 当前跟踪的去重会话数（即不同用户消息数）。 */
    get size() {
